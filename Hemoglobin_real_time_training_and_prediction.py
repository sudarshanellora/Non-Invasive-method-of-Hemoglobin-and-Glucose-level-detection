...
def real_time_prediction(model_path):
    df =  pd.read_csv(r'C:\Users\sudar\major\v.csv',converters={'ir': literal_eval,'red': literal_eval})
...
#     predicted_output=model.predict()
    ...
    predicted_output=loaded_model.predict(scaled_data)
    output=scaler.fit_transform(np.array(all_data()['Hemoglobin level g/dL'].values).reshape(-1,1))
    output=np.append(output,predicted_output)
    # output[0]=predicted_output[0]
#     print(output)
    return f"Your Hemoglobin level is : {round(scaler.inverse_transform(output.reshape(-1,1))[-1][0],2)} mg/dL"
